...
function facturaElectronica(req, res) {
    ...
    const xmlObject = {
        factura: {
            ...
            infoTributaria: {
                ambiente: "1",
                tipoEmision: "1",
                razonSocial: "Sempértegui Regalado Margarita Soledad",
                nombreComercial: "REDLAB",
                ruc: "1706610738001",
                ...
                dirMatriz: "SANCHEZ DE ORELLANA Y EMILIO SANDOVAL",
                regimenMicroempresas: "CONTRIBUYENTE RÉGIMEN MICROEMPRESAS"
            },
            infoFactura: {
                fechaEmision: "10/04/2021",
    ...
    var archivo = "factura.xml";

    fs.writeFileSync(archivo, xml);
    console.log('Se ha escrito en el archivo');

    fs.readFile('C:/DesarrolladorJunior/Proyectos EN CURSO/Facturacion REDLAB/Firma Electronica/FirmaCorrecta.pfx', (err, data) => {
        if (err) throw err;
        var archivop12 = data;
        firmarComprobante(archivop12, 'Solsito2204', xml);
    });

    res.status(200).send({ message: "Factura electronica generada." });
}
